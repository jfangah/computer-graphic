(function(imageproc) {
    ...
    imageproc.useComicPalette = function(inputData, outputData, saturation) {
        for (var i = 0; i < inputData.data.length; i += 4) {
            var temp1=imageproc.fromRGBToHSL(inputData.data[i],inputData.data[i+1],inputData.data[i+2]);
            var H=temp1["h"];
            var S=temp1["s"];
            var L=temp1["l"];
            S=S*saturation;
        ...
        for (var i = 0; i < inputData.data.length; i += 4) {
            ...
            for (var j=0; j < palette.length; j++){
                var distance=Math.sqrt(Math.pow(inputData.data[i]-palette[j][0],2)+Math.pow(inputData.data[i+1]-palette[j][1],2)+Math.pow(inputData.data[i+2]-palette[j][2],2));
